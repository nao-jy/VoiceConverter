...
class CycleGANInfer(object):
    ...
    def infer(self, mel):
        ...
        # dummy_mask = torch.ones(x.shape).to(self.device).float()
        # t1 = time.time()
        # mel_conv, _, _ = self.generator_A2B(torch.cat((x, dummy_mask), 1))
        mel_conv, _, _ = self.generator_A2B(x)
        # t2 = time.time()
        # print(t2-t1)
...
if __name__ == "__main__":
    ...
    vocoder = VocGANInfer("vocoder/finetune_prinz.pt")
