...
class CycleGANTraining(object):
    def __init__(self, source_speaker_dir, target_speaker_dir, model_checkpoint=None):
        ...
        self.discriminator_A_d = Discriminator().to(self.device)
        self.discriminator_B_d = Discriminator().to(self.device)
        ...
        d_d_params = list(self.discriminator_A_d.parameters()) + \
                   list(self.discriminator_B_d.parameters())

        # Initial learning rates
        self.generator_lr = 2e-4  # 0.0002
        self.discriminator_lr = 1e-4  # 0.0001
        self.discriminator_d_lr = 1e-4  # 0.0001

        self.generator_optimizer = torch.optim.Adam(
            g_params, lr=self.generator_lr, betas=(0.5, 0.999))
        self.discriminator_optimizer = torch.optim.Adam(
            d_params, lr=self.discriminator_lr, betas=(0.5, 0.999))
        self.discriminator_d_optimizer = torch.optim.Adam(
            d_d_params, lr=self.discriminator_d_lr, betas=(0.5, 0.999))
    ...
    def reset_grad(self):
        ...
        self.discriminator_d_optimizer.zero_grad()
    ...
    def saveModelCheckPoint(self, epoch, PATH):
        torch.save({
            ...
            'model_discriminatorA_d': self.discriminator_A_d.state_dict(),
            'model_discriminatorB_d': self.discriminator_B_d.state_dict(),
            'generator_optimizer': self.generator_optimizer.state_dict(),
            'discriminator_optimizer': self.discriminator_optimizer.state_dict(),
            'discriminator_d_optimizer': self.discriminator_d_optimizer.state_dict(),
    ...
    def loadModel(self, PATH):
        ...
        self.discriminator_A_d.load_state_dict(
            state_dict=checkPoint['model_discriminatorA_d'])
        self.discriminator_B_d.load_state_dict(
            state_dict=checkPoint['model_discriminatorB_d'])
        self.generator_optimizer.load_state_dict(
            state_dict=checkPoint['generator_optimizer'])
        self.discriminator_optimizer.load_state_dict(
            state_dict=checkPoint['discriminator_optimizer'])
        self.discriminator_d_optimizer.load_state_dict(
            state_dict=checkPoint['discriminator_d_optimizer'])
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.start_epoch, self.num_epochs + 1)):
            ...
            # Generator Training
            self.reset_grad()
            fake_B = self.generator_A2B(torch.cat((real_A * mask_A, mask_A), 1))
            cycle_A = self.generator_B2A(torch.cat((fake_B, dummy_mask), 1))

            fake_A = self.generator_B2A(torch.cat((real_B * mask_B, mask_B), 1))
            cycle_B = self.generator_A2B(torch.cat((fake_A, dummy_mask), 1))

            identity_A = self.generator_B2A(torch.cat((real_A, dummy_mask), 1))
            identity_B = self.generator_A2B(torch.cat((real_B, dummy_mask), 1))

            d_fake_A = self.discriminator_A(fake_A)
            d_fake_B = self.discriminator_B(fake_B)

            d_d_cycle_A = self.discriminator_A_d(cycle_A)
            d_d_cycle_B = self.discriminator_B_d(cycle_B)
            ...
            # 1st Step Adversarial Loss
            generator_loss_A2B = torch.mean((d_fake_B - 1) ** 2) / 2
            generator_loss_B2A = torch.mean((d_fake_A - 1) ** 2) / 2

            # 2nd Step Adversarial Loss
            generator_loss_A2B2A_2nd = torch.mean((d_d_cycle_A - 1) ** 2) / 2
            generator_loss_B2A2B_2nd = torch.mean((d_d_cycle_B - 1) ** 2) / 2

            # Total Generator Loss
            generator_loss = generator_loss_A2B + generator_loss_B2A + generator_loss_A2B2A_2nd + generator_loss_B2A2B_2nd + cycle_loss_lambda * cycleLoss + identity_loss_lambda * identiyLoss
            ...
            cycled_B = self.generator_A2B(torch.cat((fake_A, dummy_mask), 1))
            d_d_cycled_B = self.discriminator_B_d(cycled_B)

            cycled_A = self.generator_B2A(torch.cat((fake_B, dummy_mask), 1))
            d_d_cycled_A = self.discriminator_A_d(cycled_A)

            d_d_real_A = self.discriminator_A_d(real_A)
            d_d_real_B = self.discriminator_B_d(real_B)
            ...
            d_loss_B_real = torch.mean((d_real_B - 1) ** 2)
            d_loss_B_fake = torch.mean((d_fake_B - 0) ** 2)
            d_loss_B = d_loss_B_real + d_loss_B_fake

            # the 2nd step adverserial loss
            d_loss_A_real_d = torch.mean((d_d_real_A - 1) ** 2) / 2
            d_loss_A_cycled = torch.mean((d_d_cycled_A - 0) ** 2) / 2
            d_loss_A_2nd = d_loss_A_real_d + d_loss_A_cycled

            d_loss_B_real_d = torch.mean((d_d_real_B - 1) ** 2) / 2
            d_loss_B_cycled = torch.mean((d_d_cycled_B - 0) ** 2) / 2
            d_loss_B_2nd = d_loss_B_real_d + d_loss_B_cycled

            # Final Loss for discriminator with the second step adverserial loss
            d_loss = d_loss_A + d_loss_B + d_loss_A_2nd + d_loss_B_2nd
            ...
            self.discriminator_d_optimizer.step()
