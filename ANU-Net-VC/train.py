...
class CycleGANTraining(object):
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.start_epoch, self.num_epochs + 1)):
            ...
            # Generator Training
            self.reset_grad()
            fake_B, att_B = self.generator_A2B(real_A)
            cycle_A, _ = self.generator_B2A(fake_B)

            fake_A, att_A = self.generator_B2A(real_B)
            cycle_B, _ = self.generator_A2B(fake_A)

            identity_A, _ = self.generator_B2A(real_A)
            identity_B, _ = self.generator_A2B(real_B)
            ...
            # Attention Loss
            attentionLoss = torch.mean(torch.abs(att_A)) + torch.mean(torch.abs(att_B))

            # Total Generator Loss
            generator_loss = generator_loss_A2B + generator_loss_B2A + cycle_loss_lambda * cycleLoss + identity_loss_lambda * identiyLoss + attention_loss_lambda * attentionLoss
            ...
            fake_A, _ = self.generator_B2A(real_B)
            d_fake_A = self.discriminator_A(fake_A)

            fake_B, _ = self.generator_A2B(real_A)
            d_fake_B = self.discriminator_B(fake_B)
